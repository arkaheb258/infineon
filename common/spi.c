...
void spi_reset(UNSIGNED8 port){
	UNSIGNED16 j = 0;
	spi_setDiagInput(port);
	spi_writePin(port, SPI_CS0, 1);
	spi_writePin(port, SPI_RESET, 0);
	for (j=0;j<0xff;j++) //0xff = 11us; 0xffff = 3ms
		_nop();
	spi_writePin(port, SPI_RESET, 1);
}
...
void spi_send(UNSIGNED8 port, UNSIGNED8 cs, UNSIGNED16 *uwTxData, UNSIGNED16 *uwRxData, UNSIGNED16 uwDataSize){
	...
	switch (port){
		case U2C0 :
			...
			while (uwDataSize > ptr){
				if (uwDataSize - ptr > SPI_FIFO_SIZE){
					SPI_FILL_TX(U2C0, (uword*)(uwTxData + ptr), SPI_FIFO_SIZE);
					while(SPI_FILLING_RX(U2C0) < SPI_FIFO_SIZE);
					for (i=0;i<SPI_FIFO_SIZE;i++)
						uwRxData[i + ptr] = SPI_GET_RX(U2C0);
					ptr += SPI_FIFO_SIZE;
				} else {
					SPI_FILL_TX(U2C0, (uword*)(uwTxData + ptr), uwDataSize - ptr);
					while(SPI_FILLING_RX(U2C0) < uwDataSize - ptr);
					for (i=0;i<uwDataSize - ptr;i++)
						uwRxData[i + ptr] = SPI_GET_RX(U2C0);
					ptr = uwDataSize;
				}
	...
	switch (port){
		case U2C0 :
			switch (pin){
				...
				case SPI_RESET :
					if (state)
						IO_vSetPin(SPI_RESET_0);
					else
						IO_vResetPin(SPI_RESET_0);
				break;
			}
		break;
	}
